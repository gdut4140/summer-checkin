...
const AccordionGallery = ({
  ...
  const baseActive = controlledActive !== undefined ? controlledActive : internalActive;
  const active = hoverActive !== null ? hoverActive : baseActive;
      ...
      panels.forEach((panel, i) => {
        ...
        if (showLabels && bar && text) {
          if (isActive) {
            ...
            tl.to([bar, text], { opacity: 0, x: -10, duration: dur * 0.6, ease }, 0);
          }
        }
      });
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const usable = Math.max(total - gap * (count - 1), 80);
      const size = Math.max(100, usable * Math.min(Math.max(expandRatio, 0.2), 0.9) * 1.15);
  ...
  const handleKeyDown = (i: number, e: React.KeyboardEvent) => {
    if (e.key === "ArrowRight" || e.key === "ArrowDown") {
      ...
      if (controlledActive === undefined) setInternalActive(prev);
