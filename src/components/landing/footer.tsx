...
export function Footer() {
  return (
    <footer id="footer" className="border-t border-primary/15 bg-background/95 text-white">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 md:px-8">
        {/* 主区 */}
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          {/* Logo */}
          <div className="flex items-center gap-2.5">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl border border-primary/35 bg-primary/15">
            ...
            {steps.map((step, idx) => {
              ...
              return (
                <div key={step.label} className="flex items-center">
                  ...
                  {idx < steps.length - 1 && (
                    <div className="h-px w-6 bg-gradient-to-r from-primary/50 to-primary/15 sm:w-10" />
                  )}
                </div>
              );
            })}
